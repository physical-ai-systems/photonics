...
class MaterialDataset(Dataset):
    def __init__(self,
        ...
        self.validate_num_layers(self.structure_layers)
    ...
    def __getitem__(self, idx):
        ...
        material_choice = torch.randint(0, self.num_materials + 1, (self.batch_size, self.structure_layers), dtype=torch.long, device=self.device)
        ...
        for k in range(1, self.num_materials + 1):
            ...
            if mask.any():
                ri_vals = ri_stack[k-1].unsqueeze(0).unsqueeze(0).expand(self.batch_size, self.structure_layers, -1)
                refractive_indices[mask] = ri_vals[mask]

        t_raw = torch.rand(self.batch_size, self.structure_layers, device=self.device)
