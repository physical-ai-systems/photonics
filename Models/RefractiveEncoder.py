...
class RefractiveEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.name = config.get('name', 'RefractiveEncoder')
        ...
        if 'num_latent_tokens' in config:
        ...
        # Output complex refractive index (n, k) for each layer and each wavelength point
        self.material_head = nn.Linear(self.width * self.num_latent_tokens, self.structure_layers * self.spectrum_len * 2)
    ...
    def forward(self, spectrum, latent_tokens=None):
        ...
        thickness = self.thickness_head(flat_latents).squeeze(-1)  # (B, Structure_Layers)
