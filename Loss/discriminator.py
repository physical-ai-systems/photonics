...
TODO: Add reference to Mark Weber's tech report on the improved discriminator
architecture.
"""
import functools
import math
...
class BlurBlock(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ic, ih, iw = x.size()[-3:]
        pad_h = self.calc_same_pad(i=ih, k=self.kernel.shape[-2], s=2)
        pad_w = self.calc_same_pad(i=iw, k=self.kernel.shape[-1], s=2)
        x = F.pad(x, [pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2])
...
class NLayerDiscriminator(torch.nn.Module):
    def __init__(
        ...
        assert blur_kernel_size in [3, 4, 5], "Blur kernel size must be in [3,4,5]"
        assert not (blur_resample and blur_kernel_size <= 5), "Blur kernel size must be in [3,5] when sampling]"
        ...
        for i_level in range(num_stages):
            ...
            block = torch.nn.Sequential(
                Conv2dSame(in_channels, out_channels, 4, stride=1 if blur_resample else 2),
                BlurBlock(BLUR_KERNEL_MAP[blur_kernel_size]) if blur_resample else nn.Identity(),
